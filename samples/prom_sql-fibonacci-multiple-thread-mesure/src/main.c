...
// wasmのmain関数を呼び出す
void *
wamr_thread_main(wasm_exec_env_t exec_env, void *arg)
{
    ThreadArgs *thread_arg = (ThreadArgs *)arg;
    wasm_module_inst_t module_inst = get_module_inst(exec_env);
    wasm_function_inst_t func;
    ...
    func = wasm_runtime_lookup_wasi_start_function(module_inst);
    if (!func) {
...
/*SQLbench-wasmがスレッドで動かないため、監視関数をスレッド化*/
void *
wamr_thread_mem_use(void *arg)
{
...
void *
wamr_thread_func(void *arg)
{
    wasm_exec_env_t exec_env = (wasm_exec_env_t)arg;
    wasm_module_inst_t module_inst = get_module_inst(exec_env);
    wasm_function_inst_t func1;

    // sqliteを引数ありで実行する --num=10000
    // func1 = wasm_runtime_lookup_wasi_start_function(module_inst);

    func1 = wasm_runtime_lookup_function(module_inst, "sum", NULL);
    if (!func1) {
        printf("failed to lookup function sum");
        // goto fail5;
    }
    ...
    //"--num=10000"をargvに格納
    // int sql_argc = 1;
    // char sql_arg[100] = "--num=10000";
    // uint32 sql_argv[2];
    // sql_argv[0] = sql_argc;
    // sql_argv[1] = sql_arg;
    printf("start wamr_thread_func\n");
    if (!wasm_runtime_call_wasm(exec_env, func1, 0, NULL)) {
        printf("%s\n", wasm_runtime_get_exception(module_inst));
    }
...
main(int argc, char *argv[])
{
    app_argc = argc;
    app_argv = argv;
    // char *wasm_file = "wasm-apps/test.wasm"; // 二つの引数を足して返すz
    char *wasm_file = "wasm-apps/fibonacci.wasm"; //
    // char *wasm_file = "../sqlite-bench.wasm";
    uint8 *wasm_file_buf = NULL;
    uint32 wasm_file_size, wasm_argv[2], i, threads_created;
    uint32 stack_size = 100 * 1024 * 1024, heap_size = 16 * 1024;
    ...
    // wasm_application_execute_main(get_module_inst(exec_env), 0, NULL);
    ...
    */
    ...
    uint32 stack_size_other = 100 * 1024 * 1024, heap_size_other = 4 * 1024;
    ...
    int thread_ret = 0;
    // thread_ret = pthread_create(&tid_other1, NULL,
    // &wasm_runtime_measure_mem_use, exec_env_other);
    thread_ret =
        pthread_create(&tid_other1, NULL, wamr_thread_mem_use, exec_env_other);
    if (thread_ret != 0) {
    ...
    // // exec_envのスレッド実行
    // pthread_t tid_other2;
    // int thread_ret2 = 0;
    // thread_ret2 =
    //     pthread_create(&tid_other1, NULL, wamr_thread_func, new_exec_env1);
    // if (thread_ret2 != 0) {
    //     printf("pthread_create error\n");
    //     // return 1;
    // }
    ...
    prom_exe_env_array[1] = exec_env;
    // prom_exe_env_array[2] = exec_env_other;
    // prom_exe_env_array[0] = exec_env;
    prom_exe_env_array[2] = exec_env_other;

    prom_exe_env_array[0] = thread_arg[0].exec_env;
    prom_exe_env_array[1] = thread_arg[1].exec_env;
    prom_exe_env_array[2] = exec_env;
    prom_exe_env_array[3] = exec_env_other;

    prom_main_multi(prom_exe_env_array, prom_exec_env_num, 1);
    // prom_main_time(exec_env, 1);
    // prom_main_time(exec_env_other, 1);
    // prom_main_time(thread_arg[0].exec_env, 1);

    /*
    sqlテスト
    https://vscode.dev/github/oss-fun/wasm-sqlite-bench/blob/wasi-sqlitee/core/iwasm/common/wasm_application.c#L109-L110
    のコードを参考にした
    */

    // sqliteを引数ありで実行する --num=10000
    func_other =
        wasm_runtime_lookup_wasi_start_function(wasm_module_inst_other);
    //"--num=10000"をargvに格納
    int sql_argc = 1;
    char sql_arg[100] = "--num=10000";
    uint32 sql_argv[2];
    ...
    if (!wasm_runtime_call_wasm(exec_env_other, func_other, sql_argc,
                                sql_argv)) {
        printf("%s\n", wasm_runtime_get_exception(wasm_module_inst_other));
    }

    // wasm_application_execute_mainに変更
    // wasm_application_execute_main(wasm_module_inst_other, sql_argc,
    // sql_argv);
    ...
    // wasm_application_execute_main(wasm_module_inst_other, app_argc,
    // app_argv);
    ...
    // if (!wasm_runtime_call_wasm(exec_env_other, func_other, 0, NULL)) {
    //     printf("%s\n", wasm_runtime_get_exception(wasm_module_inst_other));
    // }
    // printf("call func sqlite: %d\n", wasm_argv_other[0]);

    /*wasm_runtime_measure_mem_use(exec_env_other);をスレッド化する*/
    thread_ret = pthread_join(tid_other1, NULL);
    if (thread_ret != 0) {
    ...
    //             wamr_thread_main, &thread_arg_other[i_other])) {
    ...
    //         thread_arg[i].exec_env = new_exec_env;
    //     else {
    //         printf("failed to spawn for exec_env\n");
    //         break;
    //     }
